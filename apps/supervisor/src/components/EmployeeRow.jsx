...
function EmployeeRow({
  ...
  const canAssignVehicle = isEligibleForVehicle && isPresent && !isAttendanceLocked && hasBeenEdited
  const canRequestLabour = isChalanMan && isPresent && !isAttendanceLocked
  ...
  const selectableVehicles = emp.assignedVehicle 
    ...
    : availableVehicles
  // Disable vehicle select if already assigned or attendance locked
  const vehicleSelectDisabled = isAttendanceLocked || !!emp.assignedVehicle
  // Disable labour buttons if already set or attendance locked
  const labourButtonsDisabled = isAttendanceLocked || !!emp.labourRequest
  // Show Edit button if employee has any values set (attendance, vehicle, labour request)
        ...
        <div className="flex gap-2 items-center justify-end">
          ...
          {emp.labourRequest && (
            <span className={`inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-lg border ${
              ...
              {emp.labourRequest === 'minimum' ? 'Min' : 'More'}
            </span>
          )}
          {canAssignVehicle && (
            <VehicleSelect
          ...
          {emp.assignedVehicle && !canAssignVehicle && (
          ...
          {hasAnyValueSet && !isAttendanceLocked && (
            <button
